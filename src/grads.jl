...
@adjoint function tensor(x; kwargs...)
    return tensor(x; kwargs...), Δ -> (collect(Δ),)
end
...
function ∇conv_data(
    ...
    s = reverse([
        NNlib.input_size(cdims)..., NNlib.channels_in(cdims), size(dy_, ndims(dy_))
    ])
    ...
    return Tensor{T, ndims(dy_)}(ptr[], on(dy_))
end
...
function ∇conv_filter(
    ...
    s = reverse([
        NNlib.kernel_size(cdims)..., NNlib.channels_in(cdims), NNlib.channels_out(cdims)
    ])
...
function NNlib.∇maxpool(
    ...
    return reshape(mp, reverse(size(mp))...)
end
...
function NNlib.∇meanpool(
    ...
    ptr = Ref(Ptr{Cvoid}())
    dy_ = if dy isa Base.ReshapedArray
        reshape(parent(dy), dy.dims...)
    else
        tensor(dy; dev = on(y))
    end
    kernel = collect(NNlib.kernel_size(pdims))
...
function ∇leaky_relu(dy::AbstractArray, x::Tensor{T, N}, slope) where {T, N}
    ...
    return Tensor{T, N}(ptr[], on(x))
end

@adjoint function NNlib.relu(t::Tensor{T, N}) where {T, N}
    return NNlib.relu(t), Δ -> (∇leaky_relu(Δ, t, zero(T)),)
end
